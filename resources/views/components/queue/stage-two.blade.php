<div x-data="{
        ...
        selectedCuratorName: '{{ old('curator_id')
            ? collect($curators['bishkek'])->concat($curators['moscow'])->firstWhere('id', old('curator_id'))['name']
            : 'Любой куратор' }}',
        searchCurator: '',
        filteredCurators() {
            return this.searchCurator
                ? allCurators.filter(c => c.name.toLowerCase().includes(this.searchCurator.toLowerCase()))
                : allCurators
        }
    }" class="space-y-8">
...
">
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
        {{-- Телефон --}}
        <div x-data="phoneMaskExtended('{{ old('phone_number') }}', '{{ old('country_code', 'kg') }}')" x-init="setPhoneMask(selectedCountry.mask); if(number) formatPhoneFromRaw()">
            ...
            <div class="relative">
                <div x-data="{ open: false }" class="absolute inset-y-0 left-0 flex items-center pl-4 pr-1">
                    ...
                    <div x-show="open" x-transition class="absolute z-20 mt-2 w-max origin-top-left rounded-md bg-white shadow-lg ring-1 ring-black ring-opacity-5 top-full left-0">
                        <div class="py-1">
                            <template x-for="country in countries" :key="country.id">
                                <a href="#" @click.prevent="selectCountry(country); open = false" class="text-gray-700 block px-4 py-2 text-sm hover:bg-gray-100" :class="{'bg-gray-100': selectedCountry.id === country.id}">
                ...
                <input type="tel" name="phone_number" id="phone_number" :placeholder="selectedCountry.placeholder" required class="input-style block w-full rounded-[24px] bg-white p-4 placeholder-gray-400 focus:border-yellow-400 focus:ring-2 focus:ring-yellow-400 focus:outline-none transition duration-150 pl-[105px]" x-model="number" @input="formatPhone" x-ref="phoneNumberInput">
                <input type="hidden" name="country_code" :value="selectedCountry.id">
            </div>
            @error('phone_number')
            <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
            @enderror
        ...
        <div class="relative max-w-sm">
            ...
            <div @click="openCurator = !openCurator" class="select-style block w-full rounded-[24px] bg-white p-4 cursor-pointer border border-gray-200">
                <span x-text="selectedCuratorName"></span>
            </div>
            <div x-show="openCurator" x-transition @click.away="openCurator = false" class="absolute z-10 w-full mt-1 bg-white rounded-[24px] shadow-lg p-2 border border-gray-200 max-h-80 overflow-y-auto">
                <input type="text" x-model="searchCurator" placeholder="Поиск куратора..." class="w-full mb-2 p-2 border border-gray-300 rounded-lg">

                <div class="p-1 font-semibold text-gray-500">Бишкек</div>
                <template x-for="curator in filteredCurators().filter(c => c.city === 'Бишкек')" :key="curator.id">
                    <div class="flex items-center justify-between p-2 hover:bg-gray-50 rounded-xl cursor-pointer"
                         @click="selectedCuratorId = curator.id; selectedCuratorName = curator.name; openCurator = false">
                ...
                <div class="p-1 font-semibold text-gray-500 mt-2">Москва</div>
                <template x-for="curator in filteredCurators().filter(c => c.city === 'Москва')" :key="curator.id">
                    <div class="flex items-center justify-between p-2 hover:bg-gray-50 rounded-xl cursor-pointer"
                         @click="selectedCuratorId = curator.id; selectedCuratorName = curator.name; openCurator = false">
                        <label class="text-base text-gray-700 w-full" x-text="curator.name"></label>
                        <input type="radio" :checked="selectedCuratorId === curator.id" class="h-5 w-5 text-yellow-500 border-gray-300 focus:ring-yellow-500">
                ...
                <div class="flex items-center justify-between p-2 hover:bg-gray-50 rounded-xl cursor-pointer mt-2"
                     @click="selectedCuratorId = 0; selectedCuratorName = 'Любой куратор'; openCurator = false">
                    <label class="text-base text-gray-700 w-full">Любой куратор</label>
                    <input type="radio" :checked="selectedCuratorId === 0" class="h-5 w-5 text-yellow-500 border-gray-300 focus:ring-yellow-500">
                </div>
            </div>
        </div>
    </div>

    <h3 class="text-lg font-semibold text-gray-900">Паспортные данные</h3>
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
        ...
        <div x-data="{ swapped: false }" class="relative">
            ...
            <button type="button" class="absolute inset-y-0 left-0 flex items-center pl-4 pr-3" @click="swapped = !swapped">
                <svg width="20" height="16" viewBox="0 0 20 16" fill="none"><path d="M6 16L4.6 14.575L7.175 12H0V10H7.175L4.6 7.425L6 6L11 11L6 16ZM14 10L9 5L14 0L15.4 1.425L12.825 4H20V6H12.825L15.4 8.575L14 10Z" fill="#000"/></svg>
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 gap-4 mt-8">
        <div>
            ...
            <input type="text" name="issued_by" x-model="issued_by" id="issued_by" placeholder="Кем выдан *" required class="input-style block w-full rounded-[24px] bg-white p-4 placeholder-gray-400 focus:border-yellow-400 focus:ring-2 focus:ring-yellow-400 focus:outline-none">
            @error('issued_by')
            <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
            @enderror
        </div>
        ...
        ">
            ...
            <div class="relative">
                ...
                <input type="text" name="issue_date" x-ref="issueDate" x-model="issue_date" placeholder="Дата выдачи *" required class="datepicker input-style block w-full rounded-[24px] bg-white p-4 placeholder-gray-400 pl-[70px] focus:border-yellow-400 focus:ring-2 focus:ring-yellow-400 focus:outline-none">
                @error('issue_date')
                <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                @enderror
            </div>
...
<script>
    const bishkekCurators = @json($curators['bishkek']);
    const moscowCurators = @json($curators['moscow']);
    const allCurators = [
        ...bishkekCurators.map(c => ({ ...c, city: 'Бишкек' })),
        ...moscowCurators.map(c => ({ ...c, city: 'Москва' }))
    ];
</script>
...
<script>
    ...
    function phoneMaskExtended(initialNumber = '', initialCountryId = 'kg') {
        return {
            ...
            formatPhone(e) {
                const raw = e ? e.target.value : this.number;
                const cleaned = raw.replace(/\D/g, '').slice(0, this.selectedCountry.maxlen);
                let formatted = '';
                let ci = 0;
                const mask = this.currentMask;
                for (let i = 0; i < mask.length; i++) {
                    if (ci >= cleaned.length) break;
                    if (mask[i] === '#') {
                        formatted += cleaned[ci];
                        ci++;
                    } else {
                        formatted += mask[i];
                    }
            ...
            formatPhoneFromRaw() {
                const cleaned = this.number.replace(/\D/g, '').slice(0, this.selectedCountry.maxlen);
                let formatted = '';
                let ci = 0;
                const mask = this.currentMask;
                for (let i = 0; i < mask.length; i++) {
            ...
            init() {
                const found = this.countries.find(c => c.id === initialCountryId || c.code === initialCountryId);
                if (found) {
                    this.selectedCountry = found;
                    this.currentMask = found.mask;
                }
                if (this.number) {
                    this.formatPhoneFromRaw();
                }
            }
        }
    }
    document.addEventListener('alpine:init', () => {
        Alpine.data('phoneMaskExtended', phoneMaskExtended);
    });
