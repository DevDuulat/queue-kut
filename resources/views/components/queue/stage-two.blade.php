...
<h3 class="text-lg font-semibold text-gray-900 mb-4">Личные данные</h3>
    ...
    @php
        ...
        $oldCountryCode = old('country_code', 'kg');
    @endphp

<div class="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-8">
    <div x-data="phoneMaskExtended('{{ $oldPhone }}', '{{ $oldCountryCode }}')" x-init="setPhoneMask(selectedCountry.mask); if(number) formatPhoneFromRaw()" >
    ...
    <div x-data="{
            ...
            selectedCuratorName: '{{ old('curator_id') ? collect($curators)->firstWhere('id', old('curator_id'))['name'] : 'Любой куратор' }}',
            search: '',
            filteredCurators() {
                return this.search
        ...
        <div x-show="open" x-transition @click.away="open = false" class="absolute z-10 w-full mt-1 bg-white rounded-[24px] shadow-lg p-2 border border-gray-200 max-h-60 overflow-y-auto">
            ...
            <div class="flex items-center justify-between p-2 hover:bg-gray-50 rounded-xl cursor-pointer" @click="selectedCuratorId = 0; selectedCuratorName = 'Любой куратор'; open = false">
