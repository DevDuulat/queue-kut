...
<div class="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-8">
    ...
    @php
        $oldCuratorId = old('curator_id');
        $oldCuratorName = 'Любой куратор';

        if ($oldCuratorId) {
            $found = collect($curators)->firstWhere('id', $oldCuratorId);
            if ($found) {
                $oldCuratorName = $found['name'];
            }
        }
    @endphp
    ...
    <div x-data="{
        open: false,
        selectedCuratorId: {{ $oldCuratorId ?? 'null' }},
        selectedCuratorName: '{{ $oldCuratorName }}',
        search: '',
        filteredCurators() {
            return this.search
